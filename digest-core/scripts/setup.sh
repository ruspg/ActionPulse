...
prompt_choice() {
    ...
    printf -v "$var_name" '%s' "${selected_option:-$default}"
}
...
collect_llm_config() {
    ...
    # Timeout
    while true; do
        prompt_with_default "Timeout seconds" "120" "LLM_TIMEOUT"
...
generate_env_file() {
    ...
    # Use printf to safely write values containing $, ", ` characters
    {
        printf '# ActionPulse Environment Variables\n'
        printf '# Generated by setup.sh on %s\n\n' "$(date)"
        printf '# EWS Configuration\n'
        printf 'export EWS_PASSWORD="%s"\n' "$(printf '%s' "$EWS_PASSWORD" | sed 's/["\\`$]/\\&/g')"
        printf 'export EWS_USER_UPN="%s"\n' "$EWS_USER_UPN"
        printf 'export EWS_USER_LOGIN="%s"\n' "$EWS_LOGIN"
        printf 'export EWS_USER_DOMAIN="%s"\n' "$EWS_DOMAIN"
        printf 'export EWS_ENDPOINT="%s"\n' "$EWS_ENDPOINT"
        printf '\n# LLM Configuration\n'
        printf 'export LLM_TOKEN="%s"\n' "$(printf '%s' "$LLM_TOKEN" | sed 's/["\\`$]/\\&/g')"
        printf 'export LLM_ENDPOINT="%s"\n' "$LLM_ENDPOINT"
        printf '\n# Mattermost delivery\n'
        printf 'export MM_WEBHOOK_URL="%s"\n' "$MM_WEBHOOK_URL"
    } > "$env_file"
...
$(if [[ -n "$EWS_CA_CERT" ]]; then echo "  verify_ca: \"$EWS_CA_CERT\""; fi)
  autodiscover: false
  folders: [$(echo "$EWS_FOLDERS" | sed 's/[[:space:]]*,[[:space:]]*/", "/g; s/^/"/; s/$/"/' )]
...
show_summary() {
    ...
    echo "   curl -s -X POST -H 'Content-Type: application/json' -d '{\"text\":\"ActionPulse webhook ping\"}' \"\$MM_WEBHOOK_URL\""
